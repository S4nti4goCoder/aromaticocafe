...
export function ReceiptModal({
  ...
  const RECEIPT_STYLES =
    "* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Courier New', monospace; font-size: 12px; width: 280px; padding: 8px; color: #000; }";

  const printReceipt = () => {
    const printContent = receiptRef.current;
    if (!printContent) return;
    printHtml({
      title: `Recibo #${String(sale.sale_number ?? 0).padStart(5, "0")}`,
      styles: RECEIPT_STYLES,
      bodyHtml: printContent.innerHTML,
    });
  };

  // "Imprimir" prints directly; "Descargar PDF" uses the print dialog's
  // "Save as PDF" — both render the same receipt.
  const handlePrint = printReceipt;
  const handleDownloadPDF = printReceipt;
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Detalle del comprobante generado para esta venta.
          </DialogDescription>
        </DialogHeader>
              ...
              {loyalty.mode === "sellos" ? (
                <>
                  <p className="text-center font-medium">
                    Sellos: {loyalty.stamps} / {loyalty.stampsRequired}
                  </p>
                  <p className="text-center text-gray-500">
                    {loyalty.stamps >= loyalty.stampsRequired
                      ? `🎉 ¡Premio disponible: ${loyalty.reward}!`
                      : `Te faltan ${loyalty.stampsRequired - loyalty.stamps} para: ${loyalty.reward}`}
                  </p>
                </>
              ) : (
                <p className="text-center font-medium">
                  Tienes {loyalty.points} puntos
                </p>
              )}
            </>
          )}
