...
export function FloatingButtons({
  ...
  return (
    <div
      className={`fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 flex flex-col items-center gap-2.5 sm:gap-3 transition-opacity duration-200 ${hidden ? "opacity-0 pointer-events-none" : "opacity-100"}`}
    >
      {settings?.show_whatsapp_float !== false &&
        ...
        <motion.a
          ...
          whileTap={{ scale: 0.95 }}
          className="p-3 sm:p-4 rounded-full cursor-pointer"
          style={{
      ...
      >
        <ChevronDown className="h-5 w-5 sm:h-6 sm:w-6 text-black rotate-180" />
      </motion.button>
    </div>
  );
}
