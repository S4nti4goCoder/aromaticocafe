...
export function PermissionsTab({ workerId, workerRole }: PermissionsTabProps) {
  ...
  // super_admin and gerente have fixed permissions
  const isFixedRole = workerRole === "super_admin" || workerRole === "gerente";

  // gerente can't edit permissions of other gerentes or super_admin
  ...
  const handleToggle = (
    ...
    setPermissions((prev) => ({
      ...
      [module]: {
        ...prev[module],
        [field]: value,
        // Enabling any permission auto-enables "view"
        can_view:
          field === "can_view"
            ? value
            : value
