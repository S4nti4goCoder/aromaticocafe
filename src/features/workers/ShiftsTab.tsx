...
export function ShiftsTab() {
  ...
  const weekLabel = (() => {
    ...
    return `${fmt(s)} — ${fmt(e)}`.replace(/\./g, "");
  })();

  const today = localDateString();

  // Normalize time: "6:00" → "06:00", "14" → "14:00"
  ...
  // Normalize date: "2026-04-10", "10/04/2026", or an Excel serial
  const normalizeDate = (
    v: string | number,
    ...
    reader.onload = async (evt) => {
      ...
      const parsed = rows.map((row) => {
        ...
        const date = normalizeDate(
          (row["Fecha"] ?? row["fecha"] ?? "") as string | number,
          XLSX,
        );
                          ...
                          dayShifts.map((s) => (
                            ...
                            >
                              <div
                                ...
                                className="w-full text-[10px] rounded bg-primary/10 text-primary px-1 py-0.5 mb-0.5 hover:bg-primary/20 transition-colors group relative cursor-pointer"
                              >
                                {s.start_time.slice(0, 5)}-
                                {s.end_time.slice(0, 5)}
                                <button
                ...
                <SelectContent position="popper">
                  {activeWorkers.map((w) => (
                    <SelectItem key={w.id} value={w.id}>
                      {w.full_name} ({ROLE_LABELS[w.role]})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
