...
export function AccessTab({ worker }: AccessTabProps) {
  ...
  const handleCreateAccess = async () => {
    ...
    // The user is created server-side by an Edge Function using the Admin API.
    // This keeps the admin's session intact and validates the role on the server.
    const { data, error } = await supabase.functions.invoke(
      "create-worker-account",
      { body: { email, password, worker_id: worker.id, role: worker.role } },
    );
