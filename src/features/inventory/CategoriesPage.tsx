...
import {
  buildCategoriesImportConfig,
  useCategoriesImportDone,
} from "@/features/import/configs/categoriesImport";
...
export function CategoriesPage() {
  ...
  const onImportDone = useCategoriesImportDone();
  const importConfig = buildCategoriesImportConfig({
  ...
  return (
    <div className="space-y-6">
      ...
      {(() => {
        const productCount = confirmDelete
          ? productCounts[confirmDelete.id] ?? 0
          : 0;
        ...
        return (
          <ConfirmDialog
            ...
            title={hasProducts ? "No se puede eliminar" : "Eliminar categoría"}
            description={
              confirmDelete
                ? hasProducts
                  ? `"${confirmDelete.name}" tiene ${productCount} producto(s) asociado(s). Reasigna o elimina los productos antes de poder eliminar la categoría.`
                  : `¿Seguro que quieres eliminar "${confirmDelete.name}"? Esta acción no se puede deshacer.`
                : ""
            }
            confirmLabel={hasProducts ? "Entendido" : "Eliminar"}
            destructive={!hasProducts}
