...
import {
  buildProductsImportConfig,
  useProductsImportDone,
} from "@/features/import/configs/productsImport";
...
export function ProductsPage() {
  ...
  const onImportDone = useProductsImportDone();
  const importConfig = buildProductsImportConfig({
    existingProductNames: products.map((p) => p.name),
    categoryByName: new Map(categories.map((c) => [normalizeName(c.name), c.id])),
    onDone: () => {
      onImportDone();
      setImportOpen(false);
    },
  ...
  // Margin label for admin/gerente when a cost is known for the product.
  const marginLabel = (price: number, cost: number) => {
    const margin = price - cost;
    const pct = price > 0 ? (margin / price) * 100 : 0;
    return `Margen ${formatCurrency(margin)} (${pct.toFixed(0)}%)`;
  };
  ...
  // Active discounts per product (direct or via its category)
  ...
  // Category promos act as a fallback for products without a direct one
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="w-full rounded-md border bg-card px-3 py-2 text-sm cursor-pointer sm:w-auto"
        >
                ...
                {paginatedItems.map((product, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-medium">
                      ...
                      {canSeeCost && costMap[product.id] !== undefined && (
                        <span className="block text-xs font-normal text-muted-foreground">
                          {marginLabel(product.price, costMap[product.id])}
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      ...
                      <div className="absolute bottom-2 right-2 left-2 flex justify-end">
                        <Badge
                          className={`backdrop-blur-sm shadow-sm ${
                            product.deactivated_by_stock
                              ? "bg-amber-500/90 text-amber-50 border-transparent hover:bg-amber-500/90"
                              : "bg-muted/90 text-muted-foreground border-transparent hover:bg-muted/90"
                          }`}
                        >
                          {product.deactivated_by_stock
                            ? "Sin stock"
